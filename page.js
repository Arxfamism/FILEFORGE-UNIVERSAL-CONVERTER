...
export default function Home() {
  ...
  async function convertFiles() {
    ...
    try {
      const converted = [];
      if (output === 'PDF' && files.every((file) => file.type.startsWith('image/'))) {
        converted.push({ name: 'fileforge-images.pdf', blob: await imagesToPdf(files, quality) });
      } else if (['PNG', 'JPG', 'WEBP'].includes(output)) {
      ...
      } else if (output === 'CSV' || output === 'XLSX') {
        for (const file of files) {
  ...
  return (
    <main className={dark ? 'site-shell dark-mode' : 'site-shell light-mode'}>
      ...
      <header className="topbar container">
        <a className="brand" href="#home" aria-label="FileForge home">
      ...
      <section className="results-section container">
        ...
        <div className="section-heading downloads-heading"><div><span className="section-kicker">OUTPUT</span><h2>Downloads <em>{results.length}</em></h2></div></div>
      ...
      <footer id="contact" className="footer container"><a className="brand" href="#home"><span className="brand-mark">⬡</span><span>File<span>Forge</span></span></a><p>© 2026 FileForge. All rights reserved.</p><div><a href="#about">Privacy</a><a href="#about">Terms</a><a href="#contact">Contact</a></div></footer>
    </main>
  );
}
